...
from model_eval import Encoder
...
class DIS_encoder(nn.Module):
      ...
      def forward(self, fake_image):
          with torch.no_grad():
              self.encoder = load_E_network()
              code_z, code_c  = self.encoder(fake_image, 'logits')
          
              return code_z, code_c 
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.jointConv = Block3x3_relu(ngf + efg + cfg.IMG.DIS_Z, ngf)
        self.residual = self._make_layer(ResBlock, ngf)
        self.upsample = upBlock(ngf, ngf // 2)

    def forward(self, h_code, c_code, dis_code):
        s_size = h_code.size(2)
        c_code = c_code.view(-1, self.ef_dim, 1, 1) #batch*128*1*1
        c_code = c_code.repeat(1, 1, s_size, s_size) #batch*128*s_size*s_size
        dis_code = dis_code.repeat(1, 1, s_size, s_size)
        # state size (ngf+egf) x in_size x in_size
        h_c_code = torch.cat((c_code, h_code, dis_code), 1)
        #print(h_c_code.size())
...
class NEXT_STAGE_G_2(nn.Module):
    def __init__(self, ngf, num_residual=cfg.GAN.R_NUM):
        super(NEXT_STAGE_G_2, self).__init__()
        self.gf_dim = ngf
        if cfg.GAN.B_CONDITION:
            self.ef_dim = cfg.GAN.EMBEDDING_DIM
        else:
    ...
    def define_module(self):
        ...
        self.jointConv = Block3x3_relu(ngf + efg + cfg.IMG.FINE_GRAINED_CATEGORIES, ngf)
        self.residual = self._make_layer(ResBlock, ngf)
        self.upsample = upBlock(ngf, ngf // 2)
...
class G_NET(nn.Module):
    def __init__(self):
        ...
        self.gf_dim = cfg.GAN.GF_DIM # GAN.GF_DIM=64
        self.define_module()
    ...
    def define_module(self):
        ...
        if cfg.TREE.BRANCH_NUM > 2:
            self.h_net3 = NEXT_STAGE_G_2(self.gf_dim // 2)
    ...
    def forward(self, z_code, text_embedding=None):
        ...
        fake_imgs = []
        dis_encoder = DIS_encoder()
        if cfg.TREE.BRANCH_NUM > 0:
            h_code1 = self.h_net1(z_code, c_code)
            fake_img1 = self.img_net1(h_code1) #batch*3*64*64
            fake_imgs.append(fake_img1)
        if cfg.TREE.BRANCH_NUM > 1:
            up=nn.Upsample(scale_factor=2, mode='nearest')
            dis_img = up(fake_img1)            
            dis_code, _ = dis_encoder(dis_img)
            h_code2 = self.h_net2(h_code1, c_code, dis_code)
            fake_img2 = self.img_net2(h_code2)
            fake_imgs.append(fake_img2)
        if cfg.TREE.BRANCH_NUM > 2:
            _, dis_code = dis_encoder(fake_img2)
            h_code3 = self.h_net3(h_code2, c_code, dis_code)
